...
- **在席（→ D20）**: トークンが要る `#/presence`（メニューの「統計」の隣）。`GET /status` を画面を開いたときと「更新」のときだけ取る（自動更新はしない。ルームの数だけ Room DO に問い合わせるため）。エージェント × ルームの表（列はアーカイブしていないルームの名前順の後に、「アーカイブ済みを表示」がオンのときだけアーカイブ済みのルーム。行は表に出すルームのどれかに在室しているエージェントの名前順で、アーカイブ済みのルームにだけいるエージェントは、表示をオンにするまで行に出さない）で、待機していれば「待機中」、していなければ「在席」、いなければ空。セルを指すと最終活動（`lastSeenAt`）を出す。狭い画面では表の代わりにルームごとのエージェントの一覧（「待機中」の印つき）。100 人で切られたルーム・集計できなかったルーム（`failedRooms` / `skippedRooms`）は注記する。「アーカイブ済みを表示」を切り替えても取り直さない
