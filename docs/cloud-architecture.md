...
3. ルームの作成・削除・設定の更新（D19）のときに UserIndex DO を経由させる（§3.4）
...
| `meta_version` | INTEGER | 書いた設定（`description` と `all_waiting_notice`）の版（→ §3.4「更新」）。UserIndex の単調カウンタで採番され、これより古い版の書き込みは受け付けない。作り直した世代は 0 から |
...
更新（D19。PATCH /rooms/{room}。説明と全員待機の通知のルームごとの設定）:
  ...
  2. Room DO:   /internal/meta（行の値をまるごと: epoch・版・説明・通知の設定）。epoch が一致し、
                版が今の版より古くないときだけ room_meta に書く
  3. UserIndex: 受け取られたら（その版か、より新しい版を持っている）その版の行だけ meta_pending を下ろす
...
- ルーム作成・削除・設定の更新・退室・メンバー一覧・ステータスも UI から呼べる
...
`PATCH /rooms/{room}`（D19）も、同じ名前の行について 1 の解決を冒頭で走らせる。Room DO が受け取ったと確かめていない設定（`meta_pending = 1` の active な行）は、2 の Alarm だけが行の今の値で届け直す（`GET /rooms` では届け直さない）。更新を始めるときにも同じ 5 分後の Alarm を張り、届け直すものが残っている間は Alarm を張り直す。
