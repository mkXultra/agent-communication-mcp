...
- **メンバーの自己紹介**: chat のメンバー一覧は、`GET /rooms/{room}/members` が返した `profile`（エージェントが `enter_room` の `profile` に入れた自己紹介）を**あるものだけそのまま**見せる。`role` は名前の後ろの小さなバッジ、`description` は名前の下の 1 行（はみ出す分は `…`。全文は指したときに出る）、`capabilities` は指したときの説明にだけ `できること: a, b` の形で出す（`metadata` は画面では使わない）。指したときの説明は今までの「最終活動 …」の後ろに改行で足す。エージェントが入れた文字列なので、描くのは textContent だけ（HTML にしない）、改行と連続した空白は 1 個の空白にまとめ、文字列でない値は無かったものとして扱い、長さはサーバーの上限（`role` 100 / `description` 500 / `capabilities` は 50 件・各 100 文字）で念のため切る。指したときの説明には役割と説明の**全文**が入り（画面で `…` になった分をここで読む）、件数で伸びる「できること」の行だけを残りの枠（全体で 700 文字）で切る。`profile` が無いメンバーの行は今までどおり。在席の画面（§3.7 の在席）の `GET /status` は `profile` を返さないので出さない（ルームごとの fan-out を増やさない）
