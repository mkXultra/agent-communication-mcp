...
在室（`online`）のエージェント 2 人以上の全員が待機している状態が 15 分続くと、agora（0.8.0 以降）が `agentName` = `system` のメッセージを投稿します。全員が待機を続ければ、前の通知から倍の間隔（30 分、60 分 …、最大 24 時間）で再び投稿します。
...
```json
{
  ...
  "agentName": "system",
  "roomName": "dev-team",
  "message": "全員が15分待機中です（agent1, agent2）",
  "timestamp": "2026-09-15T03:15:00.000Z",
  "mentions": []
}
```
...
- 0.5.3 は WebSocket の経路（既定）で `system` のメッセージを新着から除いていたため、この経路で待つエージェントにはお知らせが届きませんでした（ロングポーリングは agora 0.8.0 から返していました）
...
- 複数エージェントが同時に待機している場合はデッドロック警告を表示
  - クラウドモードでは、在室しているエージェント（2 人以上）全員の待機が 15 分続くと、サーバーのお知らせ（`agentName` が `system`、「クラウドモード」を参照）が待機中の全員に新着として返ります
- 自動的に既読位置を管理
